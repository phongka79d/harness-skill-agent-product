...
- [ ] **Step 1: Add failing tests** that alter payload threshold, criterion weights, mandatory flags, minimum scores, hard-fail rules, profile ID, and rubric hash while keeping evidence constant.
- [ ] **Step 2: Run the focused tests and confirm altered policy is accepted by the current code.**
- [ ] **Step 3: Add `load_canonical_rubric(contract, review_type)` and compare every pinned identity field plus canonical hash.**
- [ ] **Step 4: Remove reviewer-controlled policy fields from normalization; reject them when they differ from canonical values.**
- [ ] **Step 5: Run the focused tests and the existing adaptive quality suite.**
...
- [ ] **Step 1: Add failing tests** for duplicate criterion IDs, missing mandatory criteria, unknown criteria, invalid weight totals, score above maximum, below minimum with `PASS`, and hard-fail omission.
- [ ] **Step 2: Run them and verify each fails for the intended invariant.**
- [ ] **Step 3: Require criterion objects to carry ID, evidence, and achieved score; compare submitted IDs to canonical IDs exactly once.**
- [ ] **Step 4: Compute weighted score from canonical weights and derive verdict after hard-fail and mandatory checks.**
- [ ] **Step 5: Run focused tests and `python run_tests.py`.**
...
- [ ] **Step 1: Add a failing test** with five canonical tasks and only four submitted passing reviews.
- [ ] **Step 2: Run the test and verify the batch currently passes or fails for the wrong reason.**
- [ ] **Step 3: Load the canonical batch contract and require set equality and list uniqueness for expected and submitted task IDs.**
- [ ] **Step 4: Require accepted task state, matching rubric identity, integration/regression/scope checks, and no unresolved severe finding before `PASS`.**
- [ ] **Step 5: Run focused orchestration tests and full suite.**
