...
class ContractHardeningTests(unittest.TestCase):
    ...
    def test_attempt_reissue_schema_separates_identity_changes(self) -> None:
        ...
        self.assertEqual(
            schema["required"],
            ["task_id", "reason", "new_run_id", "new_attempt_id", "new_dispatch_id"],
        )
    ...
    def test_reissue_changes_all_durable_execution_bindings_together(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            queue = {
                ...
                "dispatches": [{"task_id": "T-ID-1", "dispatch_id": "DISPATCH-1", "run_id": "RUN-1", "attempt_id": "ATTEMPT-1", "plan_revision": 4, "worktree_path": "C:/work/T-ID-1", "branch_name": "agent/T-ID-1-r4"}],
                "locks": [],
            }
