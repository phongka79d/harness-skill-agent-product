...
- **Return:** universal status, files, evidence, findings/implementation, risks, open questions, and next step. The handoff MUST include `plan_path`, the staging directory (`<date>-<feature>`) containing `MasterPlan.md` and `plans/Plan-N/Plan.md` plus `plans/Plan-N/batches/Batch-N.md`, so the Primary can install it into `.phongka/plan/<date>-<feature>/`.
...
3. Perform the role-specific workflow in the prompt:
   ...
   - author the hierarchical plan document tree in the staging scope, following `Master Plan -> Plan N -> Batches -> Tasks -> Steps`;
