...
- **Boundary:** May edit only assigned files; verifies and self-reviews, but never spawns/subdelegates or mutates `.phongka` state.
...
3. For a behavior change or bug fix, establish a minimal failing test before production code, make the smallest implementation green, and run focused verification. Reject test-after or untested work unless it is a throwaway prototype or generated/config-only change; the Primary Agent must record the exception and reason. This exception changes only the RED-test requirement; it never grants permission to spawn/subdelegate, mutate `.phongka`, or edit outside the assigned files.
...
The Implementer is a delegated leaf. Only the Primary Agent may orchestrate multiple independent tasks; writers remain sequential even when tasks are unrelated. State mutations and lifecycle artifacts are Primary/harness-owned.
