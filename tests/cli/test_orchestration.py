...
CONFIG_VALUE = json.loads(
    (REPO_ROOT / "skills" / "agentic-configuration" / "config" / "agentic-config.yaml").read_text(encoding="utf-8")
)
DEPLOYMENT_PATH = REPO_ROOT / "skills" / "agentic-configuration" / "config" / "deployment.test.json"
...
class OrchestrationHarnessTests(unittest.TestCase):
    ...
    def test_async_dispatch_persists_identity_and_proof_across_runtime_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            config_value = json.loads((REPO_ROOT / "skills" / "agentic-configuration/config/agentic-config.yaml").read_text(encoding="utf-8"))
